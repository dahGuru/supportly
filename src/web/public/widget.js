(function () {
  ...
  // 1. Define Environment (Fixes 'isLocal is not defined')
  const isLocal = window.location.hostname === 'localhost';

  // 2. Configuration (Dynamic)
  const API_URL = isLocal
    ? 'http://localhost:3000/api/widget-auth'
    : 'https://supportly-mocha.vercel.app/api/widget-auth'; // Your Vercel URL

  const WS_URL = isLocal
    ? 'ws://localhost:4000'
    : 'wss://supportly-server.onrender.com'; // Your Render URL
  ...
  function appendMsg(text, type) {
    ...
    // Smart Append: If last msg was bot, add to it
